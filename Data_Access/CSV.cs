class CSV
{
            ...
            try
        {
            ...
            using(var reader = new StreamReader(filePath))
            {
                ...
                while (!reader.EndOfStream)
                {
                    var line = reader.ReadLine();
                    string[] values = line.Split(";");
                    if (line == "Date;Timeslot;Table;Name;Amount of persons;Reservationcode") { continue; }
    ...
    public static void WriteToCSVReservations(Reservation reservation, string filename)
    {
        try
        {
            ...
            // Write new data to the CSV file
            string userDataString = $"{reservation.Date};{reservation.TimeSlot};" + string.Join(",", reservation.Table) + $";{reservation.CustomerName};{reservation.AmountofPersons};{reservation.ReservationCode}\n";
